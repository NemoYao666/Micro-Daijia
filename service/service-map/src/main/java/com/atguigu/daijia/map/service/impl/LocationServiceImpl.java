...
public class LocationServiceImpl implements LocationService {
    ...
    public List<NearByDriverVo> searchNearByDriver(SearchNearByDriverForm searchNearByDriverForm) {
        ...
        //定义GEO参数，设置redis查询返回结果包含的数据
        RedisGeoCommands.GeoRadiusCommandArgs args = RedisGeoCommands.GeoRadiusCommandArgs.newGeoRadiusArgs()
                .includeDistance() //包含距离
                .includeCoordinates() //包含坐标
                .sortAscending(); //排序：升序

        // 1.GEORADIUS查询Redis获取附近范围内的信息
        GeoResults<RedisGeoCommands.GeoLocation<String>> result =
                this.redisTemplate.opsForGeo().radius(RedisConstant.DRIVER_GEO_LOCATION, circle, args);
    ...
    public BigDecimal calculateOrderRealDistance(Long orderId) {
        //MongoDB只需要根据规则命名方法就可以实现特定的查询功能了
        //查询方法名称以 get | find | read 开头
        //后续查询字段名称，满足驼峰式命名，比如OrderId
        //字段查询条件添加关键字，比如Like OrderBy Asc
        //根据时间排序位置，计算距离
